...
IUSE="debug mp3 +plugins-symlink x264"
...
src_compile() {
	...
	emake -r V=1 chrome chrome_sandbox BUILDTYPE=${buildtype} \
		rootdir="${S}" \
		CC=$(tc-getCC) \
		CXX=$(tc-getCXX) \
		AR=$(tc-getAR) \
...
src_install() {
	...
	doexe out/${buildtype}/chrome
	doexe out/${buildtype}/chrome_sandbox
	fperms 4755 ${CHROMIUM_HOME}/chrome_sandbox
	doexe out/${buildtype}/xdg-settings
	doexe "${FILESDIR}"/chromium-launcher.sh

	insinto ${CHROMIUM_HOME}
	doins out/${buildtype}/chrome.pak

	doins -r out/${buildtype}/locales
	doins -r out/${buildtype}/resources

	# chrome.1 is for chromium --help
	newman out/${buildtype}/chrome.1 chrome.1
	newman out/${buildtype}/chrome.1 chromium.1
	...
	newicon out/${buildtype}/product_logo_48.png ${PN}-browser.png
